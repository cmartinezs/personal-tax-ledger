...
export function computeFeeReceiptPreview(receipt: FeeReceiptPreviewInput, settings: FeeSettingsLike): FeeReceiptComputed {
  ...
  if (receipt.withholdingMode === 'WITHHELD_BY_RECIPIENT') return { grossAmount: gross, netAmount: gross - gross * rate, withholdingRate: rate, withheldAmount: gross * rate, ppmPaidAmount: 0 };
  if (receipt.withholdingMode === 'PPM_PAID_BY_ISSUER') return { grossAmount: gross, netAmount: gross, withholdingRate: rate, withheldAmount: 0, ppmPaidAmount: gross * rate };
...
export function computeFeeSummary<T extends FeeSummaryReceipt>(receipts: T[], settings: FeeSettingsLike): FeeSummary {
...
export function filterFeeReceipts<T extends {
  clientName?: string;
  status?: string;
  paymentStatus?: string;
  withholdingMode?: string;
